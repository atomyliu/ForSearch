...
{
    // 注意: 使用“重构”菜单上的“重命名”命令，可以同时更改代码和配置文件中的接口名“ISmsClassServ”。
    [ServiceContract]
    public interface ISmsClassServ
    {
        [OperationContract(Name = "GetList")]
        List<SmsClass> GetList(string keyword, int start, int size);
        [OperationContract(Name = "GetListWithType")]
        List<SmsClass> GetList(string keyword, string field, int start, int size);
